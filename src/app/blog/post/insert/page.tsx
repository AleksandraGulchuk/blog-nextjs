...
export default function Page() {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const uuid = uuidv4();
        fetch(`/api/posts?id=${uuid}&author=${user?.name}&title=${formData.title}&content=${content || formData.content}&date=${formData.date}`, {
            method: 'POST',
            headers: {
    ...
    const generateContent = () => {
        ...
        const requestParams = {
            contents: [
                ...
                {"parts": [{"text": PROMPT + formData?.title}]},
            ]
    ...
    return (
        <div className="bg-white p-8 rounded shadow">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <button type="submit"
                            className="bg-blue-400 text-white px-4 py-2 rounded-md bg-purple-600  hover:bg-purple-700">Submit
                    </button>
                </div>
            </form>
        </div>
